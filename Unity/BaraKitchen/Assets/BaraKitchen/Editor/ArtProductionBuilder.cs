...
public static partial class ArtProductionBuilder {
    ...
    static void Asset(UnityEngine.Object obj, string path) {
        // Mesh native vertex/skin buffers are not refreshed by CopySerialized in
        // this editor version. CreateAsset overwrites the generated file in place.
        if(obj is Mesh){AssetDatabase.CreateAsset(obj,path);AssetDatabase.ImportAsset(path,ImportAssetOptions.ForceSynchronousImport);return;}
        var previous = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
        if(previous) { EditorUtility.CopySerialized(obj,previous); EditorUtility.SetDirty(previous); UnityEngine.Object.DestroyImmediate(obj); }
    ...
    static void Build(bool rebuildCharacters) {
        ...
        foreach(var p in catalog.palette) {
            ...
            ColorUtility.TryParseHtmlString("#"+p.hex,out var color);m.color=color;m.SetFloat("_Gloss",p.name=="metal"?.30f:p.name=="water"?.28f:p.name=="ivory"?.14f:p.name=="teal"?.12f:.04f);
        ...
        if(rebuildCharacters)BuildCharacters();
            ...
            material.shader=Shader.Find("Bara Kitchen/Soft Color");material.color=Color.white;material.SetFloat("_ShadeFloor",.52f);material.SetFloat("_Wrap",.22f);material.SetFloat("_Gloss",.025f);EditorUtility.SetDirty(material);
        }
    ...
    static void Lighting() {
        ...
        var sun=new GameObject("Soft daylight").AddComponent<Light>();sun.type=LightType.Directional;sun.color=Color.white;sun.intensity=1;sun.transform.rotation=Quaternion.Euler(48,35,0);sun.shadows=LightShadows.Soft;sun.shadowStrength=.65f;sun.shadowBias=.045f;sun.shadowNormalBias=.14f;
