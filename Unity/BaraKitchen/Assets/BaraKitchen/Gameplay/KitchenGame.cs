...
namespace BaraKitchen.Gameplay {
    public sealed class KitchenGame : MonoBehaviour {
        public GameCatalog catalog; public KitchenCameraController view; public KitchenAudio sound; public KitchenHUD ui; public Camera cam; public SessionPhase phase=SessionPhase.Menu; public LevelDefinition level;
        ...
        public KitchenItem NewItem(ItemKind kind,string ingredient=null){string id=kind==ItemKind.Ingredient?"Ingredient_"+ingredient:kind==ItemKind.Dish?"UniversalDish":"fire-extinguisher";var go=Spawn(id,live.transform,Vector3.zero);var item=go.AddComponent<KitchenItem>();item.kind=kind;item.ingredient=ingredient;item.visual=go.GetComponent<VisualState>();item.SetWorldSize(Vector3.one*(kind==ItemKind.Dish?1.15f:kind==ItemKind.Ingredient?1f:1f));item.Refresh();return item;}
        public void PrepareLevel(int id){Time.timeScale=1;if(live){live.SetActive(false);Destroy(live);}guests.Clear();rack.Clear();returns.Clear();book=new OrderBook();book.Expired+=OnExpired;level=catalog.levels[id-1];training=true;remaining=level.duration*(Settings.calm?1.5f:1);elapsed=0;recipeCursor=0;customerCursor=0;arrivalTimer=.3f;saved=false;toast=null;feedback=null;feedbackTime=0;
            ...
            foreach(var s in stations){s.item=null;s.reserved=false;s.potContents=0;s.washing=0;if(s.kind=="pot"){var pot=Spawn("CookingPot",s.transform,s.slotOffset);s.pot=pot.GetComponent<VisualState>();s.hazard=pot.AddComponent<CookingHazard>();s.hazard.pot=s.pot;s.hazard.cookingSeconds=12;s.hazard.readySeconds=5;s.hazard.warningSeconds=8;s.hazard.visual=Spawn("CookingFireVisual",s.transform,s.slotOffset+Vector3.up*.19f).GetComponent<KitchenFireVisual>();s.hazard.StateChanged+=state=>{if(state==HeatState.Ready){Toast("Soup ready! Bring a clean dish.");AudioCue(880,.13f);}if(state==HeatState.Warning){Toast("Pot warning — serve it before it burns!");AudioCue(330,.18f);}if(state==HeatState.Burning){Toast("Fire! Grab the extinguisher.");AudioCue(160,.25f);}};}}
        ...
        public void Tick(float dt){if(phase!=SessionPhase.Service)return;elapsed+=dt;
            ...
            foreach(var s in stations){if(s.hazard){if(!training||s.hazard.state!=HeatState.Ready)s.hazard.Tick(dt);s.hazard.visual.Sample(elapsed,s.hazard.state,s.hazard.fireRemaining,s.hazard.WarningProgress,s.hazard.IgnitionProgress);}bool serve=s.kind=="serve"&&chef.held&&book.CanServe(chef.held.Recipe);bool target=s==chef.focused||s==chef.throwTarget&&chef.held&&chef.held.kind==ItemKind.Ingredient&&ThrowAllowed;s.SetHighlight(serve||target,serve?new Color(.35f,1,.63f,.95f):new Color(1,.76f,.25f,.88f),serve,Settings.reduceFlash);}
        }
        ...
        public bool Interact(ChefController c,KitchenStation s){if(phase!=SessionPhase.Service)return false;var held=c.held;
            ...
            if(s.kind=="bin"){if(!held)return Fail("Hold food to discard it");if(held.kind==ItemKind.Extinguisher)return Fail("Keep the extinguisher for kitchen fires");if(held.kind==ItemKind.Dish){held.ClearFood();Toast("Food discarded · dish kept");}else Destroy(c.Release().gameObject);return true;}
        ...
        bool PotInteract(ChefController c,KitchenStation s){var held=c.held;var h=s.hazard;
            ...
            if(h.state==HeatState.Extinguished){s.ResetPot();Toast("Burnt batch cleared");return true;}
