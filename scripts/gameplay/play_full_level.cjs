...
const level=Number(process.argv[2]||1),capture=process.env.BARA_CAPTURE!=='0',potRetake=process.env.BARA_TRAILER_POT_RETAKE==='1',cleanupCheck=process.env.BARA_BURNT_POT_CHECK==='1';
const layout=JSON.parse(fs.readFileSync('art/concepts/levels-round-02/levels.json'))[level-1],moduleSize=.92;
const qa=path.resolve(cleanupCheck?'art/production/gameplay-round-01/qa/burnt-pot':potRetake?'art/trailers/round-01/qa/pot-diagonal':'art/production/gameplay-round-01/qa/full-playthrough'),raw=path.resolve('.local/trailer-capture');
...
async function soup(film=false,burn=false){await use('dishes');await use(assembly);await chop('carrot',film);await use(assembly);await chop('mushroom');await use(assembly);check(!(await state()).stations.find(s=>s.id===assembly).recipe,'Uncooked carrot and mushroom remain ingredients on the plate');await use(assembly);await use('pot');check((await state()).held==='Clean dish'&&(await state()).stations.find(s=>s.id==='pot').heat==='Cooking','Plate tips prepared ingredients into the pot');if(film)await shot('cooking',async()=>{await sleep(4500)});await page.waitForFunction(()=>window.baraState.stations.find(s=>s.id==='pot').heat==='Ready',{},{timeout:18000});if(!burn){await press('e');check((await state()).held==='Woodland soup','Cooked soup collected on the universal plate')}else{await use(assembly);await use('extinguisher');await go('pot');await page.waitForFunction(()=>window.baraState.stations.find(s=>s.id==='pot').heat==='Warning',{},{timeout:16000});await shot('fire-and-rescue',async()=>{await page.waitForFunction(()=>window.baraState.stations.find(s=>s.id==='pot').heat==='Burning',{},{timeout:14000});await sleep(1000);await press('Space');await page.waitForFunction(()=>window.baraState.stations.find(s=>s.id==='pot').heat==='Extinguished',{},{timeout:14000});await sleep(900)});check((await state()).stations.find(s=>s.id==='pot').heat==='Extinguished','Warning progresses to fire and extinguisher puts it out');await cleanBurntPot();await use(assembly);await use('dishes')}}
async function cleanBurntPot(){
 const pot=async()=>(await state()).stations.find(s=>s.id==='pot');
 await press('e');check((await pot()).heat==='Extinguished'&&(await state()).held==='Fire extinguisher','E with extinguisher cannot clear burnt batch');
 await orbit(45);await screenshot('burnt-on-stove');await orbit(0);await use('extinguisher');await use('pot');
 const size=(await state()).heldScale;check((await state()).held==='Burnt pot'&&!(await pot()).potDocked&&(await pot()).potContents===12,'Pick up burnt pot and its contents');
 await orbit(45);await screenshot('carrying-burnt-pot');await orbit(0);await use('C1');check((await state()).stations.find(s=>s.id==='C1').item==='Burnt pot','Burnt pot can be put down on a counter');check(Math.abs((await state()).stations.find(s=>s.id==='C1').scale-size)<.0001,'Pot retains its world size on the counter');
 await chop('carrot');await use('pot');check((await state()).held==='Chopped carrot'&&!(await pot()).potDocked,'Missing pot blocks fresh ingredients');await use('bin');
 await use('C1');await use('pot');check((await pot()).potDocked&&(await pot()).heat==='Extinguished','Returning burnt pot without emptying cannot reset it');await use('pot');
 await use('bin');check((await state()).held==='Empty pot'&&(await pot()).heat==='Empty'&&(await pot()).potContents===0&&!(await pot()).potDocked,'Trash removes burnt contents and keeps pot');
 check(Math.abs((await state()).heldScale-size)<.0001,'Pot retains its original world size after emptying');await orbit(45);await screenshot('emptied-at-bin');await orbit(0);await use('pot');check((await pot()).potDocked&&!(await state()).held,'Empty pot returned to stove');
 snapshots.cleanup=await state();await screenshot('ready-for-reuse');
}
...
(async()=>{try{
 ...
 // The extinguisher sits on the floor without a collider; its cell is also the bin approach.
 blocked=new Set((await state()).stations.filter(s=>s.kind!=='extinguisher').map(s=>key(cell(s))));for(const b of layout.barriers||[])for(const [x,y] of b.cells||[])blocked.add(x+','+y);
 if(potRetake){await recapturePotAngles();return;}
 if(cleanupCheck){
  check(level===2,'Cleanup keyboard test uses the soup kitchen');await soup();await serve();await wash();
  await soup(false,true);await soup();await serve();await wash();
  check((await state()).served===2,'Fresh soup served after the trash trip');
  await page.keyboard.press('Escape');await sleep(250);await click('Restart kitchen');
  check((await state()).phase==='Briefing'&&(await state()).stations.find(s=>s.id==='pot').potDocked,'Restart restores one clean pot on the stove');
  check(errors.length===0,'No browser errors during cleanup');fs.writeFileSync(path.join(qa,'level-2.json'),JSON.stringify({passed:true,level,checks,snapshots,errors},null,2));console.log('BARA_BURNT_POT_OK');return;
 }
