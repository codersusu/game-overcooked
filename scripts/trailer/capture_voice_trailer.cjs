...
const apiKey=process.env.OPENAI_API_KEY||fs.readFileSync('.env','utf8').split('\n').find(l=>l.startsWith('OPENAI_API_KEY=')).split('=').slice(1).join('=').trim().replace(/^['"]|['"]$/g,'');
...
(async()=>{
 ...
 try{
  ...
  async function chat(){await g.click('AI chat · V');if((await g.state()).helpState==='setup'){await page.locator('input[aria-label="OpenAI API key"]').fill(apiKey);await g.click('Connect & chat');}await page.waitForFunction(()=>window.baraState.helpState==='connected',null,{timeout:40000});await quiet(2,20000);await page.keyboard.press('v');await page.waitForFunction(()=>window.baraState.helpState==='off',null,{timeout:16000});await g.sleep(2200);await start('chat-button');await g.click('AI chat · V');await page.waitForFunction(()=>window.baraState.helpState==='connected',null,{timeout:40000});await g.sleep(1000);await stop('chat-button');await quiet(2,20000)}
