...
**GPT-Live (`gpt-live-1`, Marin)** supplies continuous, interruptible conversation. `LIVE_PROMPT` defines Bara’s soft, bright capybara delivery, brief casual banter and playful cutest-animal reply. This is the game’s voice direction, not a trailer-only replacement voice or a pitch/speed effect. Restart the helper and start a new chat after changing this prompt; an existing Live session keeps its startup instructions. The browser creates a WebRTC offer; the private server exchanges it at `/v1/live/sessions`, then attaches a server WebSocket to the opaque session ID. Only negotiated SDP and a random local session token go to the browser. Browser media flows directly to OpenAI. Native Unity uses the loopback WebSocket, which relays PCM to the primary Live WebSocket. Live manages speech timing; there is no record/submit/transcribe/synthesize turn loop.
