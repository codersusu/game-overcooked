...
Follow the [README setup](../README.md#chat-with-bara-while-playing). Python 3.9+ and `aiohttp` from `scripts/voice/requirements.txt` are required. The first Chat opens a masked key popup. Web uses a browser-native password input for reliable paste; its keyboard/clipboard events stay out of Unity, and only confirmation transfers the value. Native Mac uses a Unity password field. `KitchenVoiceGuide` keeps the supplied key in a private non-serialized field until Forget key or app exit. Setup freezes input/timers and restores the prior state on cancellation or confirmation; normal conversation remains unpaused. Key-free helper copies and a Mac launcher are included in both downloads. The launcher uses `--player-keys-only`, ignoring `.env` and environment credentials. Developer automation may still opt into a private environment key by omitting that flag; the game UI always asks for the player’s key.
...
**GPT-Live (`gpt-live-1`, Marin)** supplies continuous, interruptible conversation. `LIVE_PROMPT` defines Bara’s soft, bright capybara delivery, brief casual banter and playful cutest-animal reply. This is the game’s voice direction, not a trailer-only replacement voice or a pitch/speed effect. Restart the helper and start a new chat after changing this prompt; an existing Live session keeps its startup instructions. The browser creates a WebRTC offer; the private server exchanges it at `/v1/live/sessions`, then attaches a server WebSocket to the opaque session ID. The browser/native client sends the player key only in the initial loopback start message. Each `LiveSession` uses its own credential for voice creation, attachment, reasoning and finalization; it never replaces a shared key. Only negotiated SDP and a random local session token return to the browser. The helper releases the session credential on close; no key is placed in kitchen context, responses, telemetry or persistence. Browser media flows directly to OpenAI. Native Unity uses the loopback WebSocket, which relays PCM to the primary Live WebSocket. Live manages speech timing; there is no record/submit/transcribe/synthesize turn loop.
...
## Second trailer: live voice companion

The [voice trailer](../art/trailers/round-02/) uses actual keyboard-controlled Unity gameplay and the GPT-Live WebRTC output track. Player questions are synthetic speech; no real microphone recordings are used. It demonstrates chopping help, actual score feedback, a timely soup reminder and casual chat, then ends with “Me! Bara, the capybara.” The closing joke follows the same authored character persona used in game.

`source/` retains the captured WebM/Opus clips, generated player WAVs, transcripts and read-only state snapshots. The edit mutes the closing laugh at the user’s request, trims waiting/backchannels, balances dialogue loudness, adds game music and places captions below the picture. It does not replace or fabricate the assistant’s answers. Three identical encoded title seconds bookend the film. The original gameplay trailer remains unchanged.

To reproduce, run `python3 scripts/trailer/prepare_voice_questions.py`, start a separate helper with `--port 54116`, then run `node scripts/trailer/capture_voice_trailer.cjs conversation`, `ending`, and `pot` sequentially. These capture commands make paid API calls and inject only synthetic microphone input into their isolated browser. `scripts/gameplay/driver.cjs` navigates through public keyboard/mouse controls. Render with `python3 scripts/trailer/render_voice_trailer.py`, check playback with `node scripts/trailer/check_voice_trailer.cjs` and check decoded frames/audio with `python3 scripts/trailer/check_voice_media.py`. `edit.json` records crops, source ranges, timing and captions; `qa/` contains verification evidence. The final 56.6-second, 1080p export played to completion without dropped frames or browser errors. All 90 opening/closing frames match, audio peaks at −1.8 dBFS, and a separate transcription of the final mix confirms the dialogue survived editing.
