...
One small **AI chat** button (V) starts a GPT-Live conversation; **End chat** / V stops it. First use asks for the player’s own OpenAI key in a masked popup, with session-only memory and a Forget key option. Entry briefly pauses service; cancel restores it. Microphone access is explicit. The player keeps moving and cooking, can interrupt naturally, and never has to type. Chat does not pause service or cancel work. The game UI remains English; Bara is instructed to follow the player's spoken language, including English and Chinese.

Bara has a soft, bright, smiling delivery, light playful intonation without audible giggles. Keep instructions clear and brief; avoid squeaking, baby talk and mechanical status announcements. Casual banter may playfully nominate Bara as the cutest capybara in the kitchen.
