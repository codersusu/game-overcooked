...
public static class ArtProductionBuilder {
    ...
    [Serializable] class Room { public int id,cols,rows; public string name; public Instance[] instances; public Seat[] seats; public Anchor[] anchors; public Character[] characters; public float[] cameraTarget; public float cameraSize; }
    ...
    public static void BuildAll() {
        ...
        foreach(var p in catalog.palette) {
            var m=new Material(Shader.Find("Standard"));m.name=p.name;
            ColorUtility.TryParseHtmlString("#"+p.hex,out var color);m.color=color;m.SetFloat("_Glossiness",.22f);m.SetFloat("_Metallic",p.name=="metal"?.12f:0f);
    ...
    static void BuildCharacters() {
        foreach(var dir in Directory.GetDirectories(Path.Combine(Source,"characters"))) {
            ...
            var material=new Material(Shader.Find("Standard")){name=id};material.mainTexture=AssetDatabase.LoadAssetAtPath<Texture2D>(texPath);material.color=Color.white;material.SetFloat("_Metallic",0);material.SetFloat("_Glossiness",.12f);material.DisableKeyword("_EMISSION");
    ...
    static void Lighting() {
        RenderSettings.ambientMode=AmbientMode.Trilight;RenderSettings.ambientSkyColor=new Color(.48f,.52f,.56f);RenderSettings.ambientEquatorColor=new Color(.37f,.39f,.35f);RenderSettings.ambientGroundColor=new Color(.24f,.21f,.18f);RenderSettings.ambientIntensity=1;
        RenderSettings.skybox=null;RenderSettings.fog=false;
        var sun=new GameObject("Soft daylight").AddComponent<Light>();sun.type=LightType.Directional;sun.color=new Color(1,.95f,.86f);sun.intensity=.70f;sun.transform.rotation=Quaternion.Euler(48,35,0);sun.shadows=LightShadows.Soft;sun.shadowStrength=.45f;sun.shadowBias=.02f;sun.shadowNormalBias=.08f;
        var fill=new GameObject("Window fill").AddComponent<Light>();fill.type=LightType.Directional;fill.color=new Color(.78f,.88f,1);fill.intensity=.15f;fill.transform.rotation=Quaternion.Euler(35,-145,0);fill.shadows=LightShadows.None;
    }
    static Camera CameraAt(Vector3 target,float size) {
        ...
        cam.transform.position=target+new Vector3(-7.4f,14,17);cam.transform.LookAt(target);cam.clearFlags=CameraClearFlags.SolidColor;cam.backgroundColor=new Color(.89f,.87f,.80f);cam.allowHDR=false;cam.allowMSAA=true;return cam;
    }
    static string BuildRoom(Room room) {
        ...
        foreach(var inst in room.instances) {
            bool customer=inst.position[2]>=(room.rows-.48f)*.8f;var go=Spawn(inst.asset,customer?cafe.transform:kitchen.transform,V(inst.position),Quaternion.Euler(0,inst.yaw,0));go.name=inst.name;
        ...
        var floor=new GameObject("Walkable floor");floor.transform.SetParent(root.transform);var col=floor.AddComponent<BoxCollider>();col.center=new Vector3(0,-.085f,(room.rows+4)*.4f);col.size=new Vector3(room.cols*.8f,.1f,(room.rows+5)*.8f);
        ...
        foreach(var ch in room.characters) if(characters.ContainsKey(ch.id)) {
            ...
            var pose=go.GetComponent<PreviewPose>();pose.seated=ch.pose=="seated";pose.walking=ch.pose=="walk";pose.animate=true;
            if(pose.seated)go.transform.localPosition-=Vector3.up*.18f;pose.ApplyPose(.4f);
        }
    ...
    static void ExportCharacterPoses(string path) {
        ...
        foreach(var pose in UnityEngine.Object.FindObjectsByType<PreviewPose>(FindObjectsSortMode.None)) {
            var renderer=pose.GetComponent<SkinnedMeshRenderer>();var mesh=new Mesh();renderer.BakeMesh(mesh);
            ...
            output.Add(new BakedCharacter {id=pose.name,vertices=verts,normals=norms,uv=mesh.uv.SelectMany(v=>new[]{v.x,v.y}).ToArray(),triangles=mesh.triangles});
