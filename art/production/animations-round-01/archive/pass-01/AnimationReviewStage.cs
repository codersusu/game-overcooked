...
namespace BaraKitchen {
    ...
    public sealed class AnimationReviewStage : MonoBehaviour {
        ...
        Vector3 throwStart;float previous=-1;
        ...
        public static float Length(string id){switch(id){case "Walk":return 2.4f;case "Dash":return 1.15f;case "CarryWalk":return 2.7f;case "Throw":return 1.7f;case "Extinguish":return 4.5f;case "CustomerEat":return 2.6f;case "Overcook":return 4.8f;default:return 3.2f;}}
        ...
        public void SetScenario(string next,string character=null) {
            ...
            playhead=0;previous=-1;held=null;food=null;foodState=null;dishState=null;hazard=null;spray=null;
            ...
            if(next=="Chop") {
                actor.transform.localPosition=new Vector3(0,0,-.05f);Make("counter",new Vector3(0,0,.90f),null,1.15f);Make("cutting-board",new Vector3(0,.68f,.60f));
                foodState=Make("Ingredient_tomato",new Vector3(0,.71f,.57f),null,.68f).GetComponent<VisualState>();food=foodState.transform;held=Make("knife",new Vector3(0,0,.085f),actor.rightToolAnchor).transform;
            }
            if(next=="Wash") {
                actor.transform.localPosition=new Vector3(0,0,-.05f);var station=Make("station-sink",new Vector3(0,0,.92f),null,1.15f);station.transform.localRotation=Quaternion.Euler(0,180,0);
                dishState=Make("UniversalDish",new Vector3(0,.692f,.53f)).GetComponent<VisualState>();dishState.SetState(5);
                held=Make("sponge",new Vector3(0,-.019f,0),actor.rightToolAnchor).transform;
            }
            if(next=="Overcook"||next=="Extinguish") {
                Make("counter",new Vector3(0,0,.8f),null,1.15f);
                var pot=Make("CookingPot",new Vector3(0,.69f,.80f)).GetComponent<VisualState>();
                var ring=Make("pot-lid",new Vector3(0,.68f,.8f));ring.transform.localScale=new Vector3(1.3f,.08f,1.3f);
                var fx=Make("CookingFireVisual",new Vector3(0,.86f,.8f)).GetComponent<KitchenFireVisual>();hazard=pot.gameObject.AddComponent<CookingHazard>();hazard.visual=fx;hazard.pot=pot;hazard.cookingSeconds=.9f;hazard.readySeconds=.9f;hazard.warningSeconds=.9f;hazard.BeginCooking();
                if(next=="Overcook"){actor.transform.localPosition=new Vector3(-.8f,0,0);actor.transform.localRotation=Quaternion.Euler(0,35,0);}
                else {
                    actor.transform.localPosition=new Vector3(0,0,-.18f);held=Make("fire-extinguisher",Vector3.zero,actor.carryAnchor).transform;spray=Make("ExtinguisherSpray",Vector3.zero).GetComponent<ExtinguisherSpray>();hazard.Tick(3);spray.range=2.5f;
                }
            }
        ...
        public static void FrameCamera(Camera cam,string scenario) {
            ...
            cam.transform.position=target+(station?new Vector3(3.4f,3.6f,3.5f):new Vector3(2.8f,2.25f,4.1f));cam.transform.LookAt(target);cam.orthographicSize=throwing?1.60f:station?1.22f:1.02f;cam.aspect=1;
        }
        public void SampleFrame(float t) {
            ...
            if(scenario=="Dash") {float travel=Mathf.SmoothStep(0,1,Mathf.InverseLerp(.08f,.55f,t));actor.transform.localPosition=new Vector3(0,0,-.55f+travel*1.1f);}
            ...
            if(scenario=="Throw"&&held) {
                ...
                else{held.SetParent(content,true);float u=Mathf.Clamp01((t-release)/.72f);held.position=Vector3.Lerp(throwStart,new Vector3(0,.683f,2.13f),u)+Vector3.up*(1.25f*4*u*(1-u));held.rotation=Quaternion.Euler(u<1?u*180:0,0,0);}
            }
            ...
            if(hazard) {
                if(scenario=="Overcook")hazard.Tick(delta);
                if(spray) {
                ...
                hazard.visual.Sample(t,hazard.state,hazard.fireRemaining);
            }
            playhead=t;
        }
