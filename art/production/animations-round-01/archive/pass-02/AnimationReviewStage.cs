...
namespace BaraKitchen {
    ...
    public sealed class AnimationReviewStage : MonoBehaviour {
        ...
        public static float Length(string id){switch(id){case "Walk":return 2.4f;case "Dash":return 1.15f;case "CarryWalk":return 2.7f;case "Throw":return 2.0f;case "Extinguish":return 4.5f;case "CustomerEat":return 2.6f;case "Overcook":return 6.2f;default:return 3.2f;}}
        ...
        public void SetScenario(string next,string character=null) {
            ...
            if(next=="Overcook"||next=="Extinguish") {
                ...
                var fx=Make("CookingFireVisual",new Vector3(0,top+.19f,.8f)).GetComponent<KitchenFireVisual>();hazard=pot.gameObject.AddComponent<CookingHazard>();hazard.visual=fx;hazard.pot=pot;hazard.cookingSeconds=1.2f;hazard.readySeconds=1.0f;hazard.warningSeconds=1.6f;hazard.BeginCooking();
                if(next=="Overcook"){actor.transform.localPosition=new Vector3(-.90f,0,-.15f);actor.transform.localRotation=Quaternion.Euler(0,0,0);}
                    ...
                    actor.transform.localPosition=new Vector3(0,0,-.18f);held=Make("fire-extinguisher",Vector3.zero,actor.carryAnchor).transform;spray=Make("ExtinguisherSpray",Vector3.zero).GetComponent<ExtinguisherSpray>();hazard.Tick(5);spray.range=2.5f;
                }
            }
