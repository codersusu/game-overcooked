...
namespace BaraKitchen {
    ...
    public static class CharacterMotion {
        ...
        static void SideStretch(Transform[] b,float phase) {
            ...
            Rot(b[3],0,0,-4-43*lift);Rot(b[5],0,0,4+43*lift);
            Rot(b[4],0,0,-5*lift);Rot(b[6],0,0,5*lift);
        }
        ...
        public static void Sample(Transform root,Transform[] b,ChefAction action,float seconds) {
            ...
            if(action==ChefAction.Chop) {
                ...
                var right=new Vector3(.30f,.615f+.105f*lift,.35f-.04f*lift);
                Arm(root,b[5],b[6],b[6].Find("Paw"),right);
                Arm(root,b[3],b[4],b[4].Find("Paw"),new Vector3(-.10f,.49f,.31f));
                Tool(root,b[12],root.InverseTransformPoint(b[6].Find("Paw").position),new Vector3(40-18*lift,0,0));
            }
            if(action==ChefAction.Wash) {
                var right=new Vector3(.20f+.095f*s,.505f+.008f*Mathf.Sin(p*2),.34f+.05f*c);
                Arm(root,b[5],b[6],b[6].Find("Paw"),right);
                Arm(root,b[3],b[4],b[4].Find("Paw"),new Vector3(-.10f,.48f,.32f));
                Tool(root,b[12],root.InverseTransformPoint(b[6].Find("Paw").position),new Vector3(0,22*s,0));
            }
            if(action==ChefAction.Extinguish) {
                var shift=new Vector3(.055f*s,0,0);b[0].localPosition+=shift;
                Rot(b[0],0,5*s,-5*s);Rot(b[1],-3,4*s);Rot(b[2],2,-3*s);
                Rot(b[7],-8);Rot(b[9],10);
                Tool(root,b[11],new Vector3(.09f,.28f,.27f)+shift,new Vector3(0,9*s,-6-5*s));
